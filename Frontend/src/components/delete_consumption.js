...
class DeleteConsumptionForm extends Component {
    ...
    render() { 
        ...
        <SplashEditConsumptionDiv>
            <Form onSubmit={(e) => this.handleDeleteConsumption(e)} 
            style={{position: "relative", width: 550, height: 550, margin: '20px'}}
            >
                {this.state.redirect}
                <FormGroup >
                    <FormSelect name="consumptionId">
                        {this.props.userInfo.consumptions.map(consumption => <option value={consumption.id}>Id: {consumption.id}, Type: {consumption.category}, Calories Intaken: {consumption.calories_intaken ? consumption.calories_intaken :'Nil'} </option>)}
                    </FormSelect>
                </FormGroup>
                    <Button className="mb-2" type="submit">Delete Consumption</Button>
            </Form> 
        </SplashEditConsumptionDiv>);
    }
}
